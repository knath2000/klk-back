...
router.get('/:id', async (req, res) => {
    try {
        const model = await modelService_1.modelManager.getModelById(req.params.id);
        if (!model) {
...
router.post('/:id/switch', async (req, res) => {
    try {
        ...
        const model = await modelService_1.modelManager.switchModel(conversationId, req.params.id);
        res.json({
            success: true,
...
router.get('/capability/:capability', async (req, res) => {
    try {
        const models = await modelService_1.modelManager.getModelsByCapability(req.params.capability);
        res.json(models);
    }
    catch (error) {
